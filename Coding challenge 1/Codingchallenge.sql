...
Select a.name,count(a1.ArtworkID) as artworkcount from Artists a left join Artworks a1 on a.ArtistID=a1.ArtistID Group by a.name order by artworkcount desc;
...
where a.Nationality in('Spanish','Dutch') Order by a1.Year ASC;
...
Select a.Name as ArtistName, TotalArtworks from (select art.ArtistID, count(art.ArtworkID) as TotalArtworks from Artworks art group by art.ArtistID)
as ArtworkCount join Artists a on a.ArtistID = ArtworkCount.ArtistID where TotalArtworks > 2; 
...
SELECT DISTINCT a.name AS artist_name from artists a join artworks art on a.artistid = art.artistid where art.categoryid = (select categoryid from artworks 
where title = 'Mona Lisa');

--20 List the names of artists and the number of artworks they have in the gallery. 
SELECT a.name AS artist_name, COUNT(art.artworkid) AS artwork_count from artists a left join artworks art on a.artistid = art.artistid
group by a.name;
